...
class psdb3(imdb):
    def __init__(self, image_set, year):
        imdb.__init__(self, 'psdb3_' + image_set)
        self._image_set = image_set
        self._data_path = os.path.join(cfg.DATA_DIR, 'psdb3')

        self._classes = ('background', 'head', 'head-shoulder', 'upperbody')
        ...
        for idx, num in enumerate(numList):
            if idx % 5 != 0:
                ...
                if len(part) == 4:
                    classId = classId + 1
                    part.append(classId)
                    if key>=0 and min(part)>=0 and classId>0:
                        ret.append(part)
                    part = []
            else:
                key = num
