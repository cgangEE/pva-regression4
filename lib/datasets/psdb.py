...
class psdb(imdb):
    ...
    def getPerson(self, s):
        numList = map(float, s.strip().split())
    ...
    def _get_annotation(self, index):
        ...
        parts = np.zeros((num_objs, 12), dtype=np.uint16)
        ...
        for i, person in enumerate(persons):
            parts[ix]  = person[1][:4] + person[2][:4] + person[3][:4]
            x1, y1, x2, y2, cls = person[0]
