...
class tsinghua(imdb):
    def __init__(self, image_set, year):
        imdb.__init__(self, 'tsinghua_' + image_set)
        self._image_set = image_set
        self._data_path = os.path.join(cfg.DATA_DIR, 'tsinghua')
    ...
    def _get_annotation(self, index):
        filename = os.path.join(self._data_path, 'label',
                index[:-15] + 'labelData.json')
        fAnnotation = open(filename, 'r')
        annotationStr = ''
        for line in fAnnotation:
            annotationStr += line

        annotation = json.loads(annotationStr)['children']
        ...
        for i, person in enumerate(annotation):
            x1 = person['mincol']
            y1 = person['minrow']
            x2 = person['maxcol']
            y2 = person['maxrow']
...
if __name__ == '__main__':
    my_cox_reid = tsinghua('test', '2014');
