...
def test_net(net, imdb, max_per_image=100, thresh=0.05, vis=False):
    ...
    for i in xrange(num_images):
        ...
        scores, boxes, part_boxes = im_detect(net, im, _t, box_proposals)
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            for k in range(3):
                part = part_boxes[k]
                cls_part_boxes = part[inds, j*4:(j+1)*4]

                cls_part_dets = np.hstack((
                        cls_part_boxes, cls_scores[:, np.newaxis])) \
                        .astype(np.float32, copy=False)
                dets_NMSed = cls_part_dets[keep, :]
                all_boxes[j+k+1][i] = dets_NMSed 
        ...
        if max_per_image > 0:
            image_scores = np.hstack([all_boxes[j][i][:, -1]
                                      for j in xrange(1, imdb.num_classes+3)])
